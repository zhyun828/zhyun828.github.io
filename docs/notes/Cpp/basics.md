...
| `>>` | 右移   | 向右移位    |       |
```c
__builtin_popcount(5) // 计算二进制中 1 的个数，5 的二进制是 101，所以结果是 2
__builtin_popcount((unsigned)j) // 计算 j 的二进制中 1 的个数，先将 j 转换为无符号整数，避免负数的二进制表示导致结果不正确
x >> 1 // 右移一位，相当于除以 2，x 的二进制表示向右移动一位，也就是把最低位去掉。
```
